...
IN_TEST_NNUNET_IMAGES = './data/test/nnUNet_raw/imagesTs/'
IN_TEST_NNUNET_LABELS = './data/test/nnUNet_raw/labelsTs/'
...
def nnunet_to_npy(IN_IMAGES, IN_LABELS, OUT_IMAGES, OUT_LABELS):
    ...
    for img in range(len(t2f_list)):
        ...
        temp_image_t1n = torch.tensor(
            temp_image_t1n, dtype=torch.float32, device=device)
        temp_image_t1c = torch.tensor(
            temp_image_t1c, dtype=torch.float32, device=device)
        temp_image_t2w = torch.tensor(
            temp_image_t2w, dtype=torch.float32, device=device)
        temp_image_t2f = torch.tensor(
            temp_image_t2f, dtype=torch.float32, device=device)
        temp_mask = torch.tensor(temp_mask, dtype=torch.long, device=device)

        # Stack the images into a single volume
        temp_combined_images = torch.stack(
            [temp_image_t1n, temp_image_t1c, temp_image_t2w, temp_image_t2f], dim=0)
        ...
        if (1 - (torch.sum(temp_mask == 0) / torch.prod(torch.tensor(temp_mask.shape)))) > 0.01:
            ...
            np.save(OUT_IMAGES+"/image_" + str(img) + ".npy",
                    temp_combined_images.cpu().numpy())
            np.save(OUT_LABELS+"/mask_" + str(img) + ".npy",
                    temp_mask.cpu().numpy().astype(np.uint8))
...
# nnunet_to_npy(IN_TRAIN_NNUNET_IMAGES, IN_TRAIN_NNUNET_LABELS,
#               OUT_TRAIN_IMAGES, OUT_TRAIN_LABELS)

print("PREPROCESS TEST CASES")
nnunet_to_npy(IN_TEST_NNUNET_IMAGES, IN_TEST_NNUNET_LABELS,
              OUT_TEST_IMAGES, OUT_TEST_LABELS)
